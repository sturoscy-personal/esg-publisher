...
class FieldCheck(object):
    ...
    def run_check(self, input_rec):
        ...
        if not src_id in self.sid_dict:
            self.publog.error(
                "Source_id {} is unregistered with the {} Controlled Vocabulary (CV). Publication halted".format(
                    src_id, self.project_str
                )
            )
            self.publog.error(
                "If you think this message has been received in error, please update your CV source repository"
            )
            raise UserWarning

        if (
            self.project_key in SOURCE_ID_LIMITS
            and len(src_id) > SOURCE_ID_LIMITS[self.project_key]
        ):
            self.publog.error(
                f"Source_id {src_id} exceeds the {SOURCE_ID_LIMITS[self.project_key]} character limit for project {self.project_str}. Publication halted."
            )
            raise UserWarning

        if not self.check_activity(src_id, act_id):
            self.publog.error(
                "Source_id {} is not registered for participation in CMIP6 activity {}. Publication halted".format(
                    src_id, act_id
                )
            )
            self.publog.error(
        ...
        if not self.check_institution(src_id, inst_id):
            self.publog.error(
                "Institution_id {} is not registered to contribute to source_id {}. Publication halted".format(
                    inst_id, src_id
                )
            )
            self.publog.info(
