...
class PublisherArgs:
    ...
    def get_args(self):
        ...
        parser = argparse.ArgumentParser(
            description="Publish data sets to ESGF databases."
        )
        ...
        parser.add_argument(
            ...
            dest="no_replica",
            action="store_true",
            help="Disable replica publication.",
        )
        parser.add_argument(
            "--esgmigrate",
            dest="migrate",
            action="store_true",
            help="Run esgmigrate before publishing, to migrate over old config. May be left with incomplete config settings.",
        )
        parser.add_argument(
            "--json",
            dest="json",
        ...
        parser.add_argument(
            ...
            dest="cmor_path",
            default=None,
            help="Path to CMIP6 CMOR tables for PrePARE. Required for CMIP6 only.",
        )
        parser.add_argument(
            "--autocurator",
            dest="autocurator_path",
            default=None,
            help="Path to autocurator repository folder.",
        )
        parser.add_argument(
            "--map",
            dest="map",
            required=True,
            nargs="+",
            help="Mapfile or list of mapfiles.",
        )
        parser.add_argument(
            "--config",
            "-cfg",
            dest="cfg",
            default=str(def_config),
        ...
        parser.add_argument(
            "--version",
            action="version",
            version=f"esgpublish v{esgcet.__version__}",
        ...
        parser.add_argument(
            ...
            dest="xarray",
            action="store_true",
            help="Use Xarray to extract metadata even if Autocurator is configured.",
        )
    ...
    def load_config(self, config_path):
        ...
        try:
            ...
            publog.error(
                "Could not open file, please provide correct path to yaml config file."
            )
            quit(1)
    ...
    def get_dict(self, fn_project):
        ...
        if os.path.isdir(config_file):
            publog.error(
                "Config file path is a directory. Please use a complete file path, exiting."
            )
            exit(1)
        ...
        if pub.data_node is None:
            try:
                ...
                publog.exception(
                    "Data node not defined. Use --data-node option or define in esg.ini."
                )
                exit(1)
        else:
        ...
        try:
            ...
            if data_roots == "none":
                publog.error(
                    "Data roots undefined. Define in config file to create file metadata."
                )
                exit(1)
        except:
            publog.exception(
        ...
        if project == "cmip6":
            if pub.cmor_path is None:
                try:
                    ...
                    publog.exception(
                        "No path for CMOR tables defined. Use --cmor-tables option or define in config file."
                    )
                    exit(1)
            else:
        ...
        if (
            project == "cmip6"
            or project == "input4mips"
            or (project in proj_config and "pid_prefix" in proj_config[project])
        ):
            try:
        ...
        if "enable_archive" in config and config.get("enable_archive", False):
            try:
                ...
                if not os.path.isdir(argdict["archive_path"]):
                    publog.exception(
                        f"Error with archive path {argdict['archive_path']}"
                    )
                    exit(1)
            except:
                publog.exception(
                    "Configuration file error: check archive (and other) settings"
                )
                exit(1)
        else:
