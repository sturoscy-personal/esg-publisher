...
def get_args():
    ...
    parser.add_argument(
        "--scan-file",
        dest="scan_file",
        help="JSON output file from autocurator.  If not set will use XArray to load data from files",
    )
    parser.add_argument(
        "--map-data",
        dest="map_data",
...
def run():
    ...
    if a.index_node is None:
        try:
            ...
            publog.exception(
                "Index node not supplied in config or command line. Exiting."
            )
            exit(1)
    else:
    ...
    if a.set_replica and a.no_replica:
        ...
        try:
            ...
            publog.exception(
                "Set_replica not defined. Use --set-replica or --no-replica or define in config file."
            )
            exit(1)
    ...
    try:
        if non_nc:
            ...
            out_json_data = mkd.get_records(
                map_json_data, scanarg, json_file, user_project=proj_config
            )
