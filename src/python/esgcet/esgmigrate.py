...
class ESGPubMigrate(object):
            ...
            if key in MIGRATE_KEYS:
                ...
                if type(cf_val) is str and len(cf_val) > 1:
                    try:
                        ...
                        self.publog.warn(
                            f"Expected JSON string for {key} could not be parsed"
                        )
                        traceback.print_exc()
                else:
    ...
    def project_migrate(self, project):
        ...
        try:
            ret["CONST_ATTR"] = {
                x[0]: x[1] for x in SP.get_options_from_table("category_defaults")
            }
        except:
            ret["CONST_ATTR"] = {}
    ...
    def migrate(self, project=None):
        ...
        if not os.path.exists(os.path.join(self.ini_path, "esg.ini")):
            self.publog.error(
                "Old config " + self.ini_path + "esg.ini not found or unreadable."
            )
            exit(1)
        ...
        if "rest_service_url" in sp:
            ...
            self.publog.warning(
                "No REST publishing url found in old config, will need to set index_node setting"
            )
        ...
        new_config = {
            ...
            "data_transfer_node": DATA_TRANSFER_NODE,
            "pid_creds": pid_creds,
            "cmor_path": cmor_path,
        }
        if project_config:
