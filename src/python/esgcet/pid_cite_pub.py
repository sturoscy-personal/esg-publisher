...
from esgcet.settings import (CITATION_URLS, HTTP_SERVICE, PID_EXCHANGE,
                             PID_PREFIX, PID_URL)
...
class ESGPubPidCite(object):
    ...
    def establish_pid_connection(self):
        ...
        try:
            ...
            self.publog.exception(
                "PID module not found. Please install the package 'esgfpid' (e.g. with 'pip install')."
            )
            exit(1)
        ...
        self.publog.debug(
            f" self.pid_connector = esgfpid.Connector(handle_prefix={self.pid_prefix}, \
        ...
        self.pid_connector = esgfpid.Connector(
            handle_prefix=self.pid_prefix,
            messaging_service_exchange_name=pid_messaging_service_exchange_name,
            messaging_service_credentials=pid_messaging_service_credentials,
            data_node=pid_data_node,
            thredds_service_path=http_service_path,
    ...
    def check_pid_connection(self, send_message=False):
        ...
        """
        pid_queue_return_msg = self.pid_connector.check_pid_queue_availability(
            send_message=send_message
        )
        if pid_queue_return_msg is not None:
            self.publog.error(
                "Unable to establish connection to PID Messaging Service. Please check your esg.ini for correct pid_credentials."
            )
    ...
    def pid_flow_code(self):
        ...
        try:
            ...
            self.publog.exception(
                "Failed to obtain dataset record: {}".format(str(self.ds_records))
            )
            exit(-1)
        ...
        if self.pid_connector:
            self.dataset_pid = (
                self.pid_connector.make_handle_from_drsid_and_versionnumber(
                    drs_id=dset, version_number=version_number
                )
            )
            self.publog.info(
                "Assigned PID to dataset %s.v%s: %s "
                % (dset, version_number, self.dataset_pid)
            )
        else:
        ...
        try:
            ...
            pid_wizard = None
            # Check connection
            pid_wizard = self.pid_connector.create_publication_assistant(
                drs_id=dset, version_number=version_number, is_replica=is_replica
            )
    ...
    def pid_unpublish(self, drs_id, version):
        ...
        try:
            self.pid_connector.unpublish_one_version(
                drs_id=drs_id, version_number=version
            )
    ...
    def update_dataset(self, index):
        ...
        if not self._disable_cite:
            ...
            citation_url = CITATION_URLS[project][keystr].format(
                dset_rec["master_id"], dset_rec["version"]
            )
    ...
    def do_pidciterewrite(self, fname):
        ...
        if type(res) is list:
            ...
            self.publog.warning(
                "Something went wrong, PID/cite information were not added"
            )
