...
class ESGPubMKDCreateIP(ESGPubMakeDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.source_ids = [
            ...
            "ERA40-CRUTS3-10",
            "ERA5",
            "ERAInterim-CRUTS3-10",
            "GECCO2",
            "GODAS",
            "JRA-25",
            "JRA-55," "JRA-55-mdl-iso",
            "MERRA",
            "MERRA-CRUTS3-10",
            "MERRA2",
            "MERRA2-ASM",
        ...
        self.publog = log.return_logger(
            "Make Dataset CREATE-IP", self.silent, self.verbose
        )
    ...
    def aggregate_datasets(self, datasets, limit=False):
        ...
        for data in datasets:
            ...
            dataset = data[idx]
            if (
                self.variable_name in dataset
                and dataset[self.variable_name] not in vids
            ):
                vids.append(dataset[self.variable_name])
            if (
                "variable_long_name" in dataset
                and dataset["variable_long_name"] not in v_long_names
            ):
                v_long_names.append(dataset["variable_long_name"])
            if (
                "cf_standard_name" in dataset
                and dataset["cf_standard_name"] not in cf_std_names
            ):
