...
class publisherClient(object):
    ...
    def __init__(
        self, cert_fn, hostname, verify=False, verbose=False, silent=False, auth=False
    ):
    ...
    def post_data(self, url, data):
        ...
        if self.use_cert:
            ...
            resp = requests.post(
                url, data=data, verify=self.verify, allow_redirects=True
            )
    ...
    def publish(self, xmldata):
        ...
        try:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    "Publish returned status code" + str(response.status_code)
                )
    ...
    def update(self, xmldata):
        ...
        try:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    "Update returned status code" + str(response.status_code)
                )
